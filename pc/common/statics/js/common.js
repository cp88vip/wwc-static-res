...
function showBetDetail(detail) {
    ...
    var txtHtml = '<h4>注单号：' + betArr['oid'] + '</h4>'
            ...
            + '<tr class=hid>'
            + '<td width=66>玩法：</td>'
            + '<td width=100>' + decodeURIComponent(betArr['pname']).replace(/复式|单式/, "单/复式") + '</td>'
            + '<td width=66>销售返点：</td>'
...
{   if(!type_flag){type_flag="trend";}
    ...
    function i_inHtml(data)
    {
        ...
        for( var i=0;i<data.length;i++ )
        {
            ...
            arr_trend_list.push("<span id=\"_m_t_"+gid+"\" name=\"type_"+data[i]["type"]+"\" onclick=\"__openWin('home2','/pc/trend/chart.html?lotteryId="+gid+"')\">"+data[i]["name"]+"</span>");
        }
    ...
    function getUrlParameterResult(paramName,url){
        ...
        try {
            ...
            if (paramUrl.length > 0){
                paramUrl = paramUrl.replace("/nonAuthority/home/openDetail?", "");
...
_common.util = {
    ...
    dialogUi:function(){
        ...
        {
            var newDiv = $("<div id=\"JS_blockPage\" class=\"JS_blockPage\"><div class=\"detail_dialog\"><h3 id=\"block_draghandler\"></h3><img src=\"/pc/common/statics/img/close.gif\" class=\"c1\" id=\"block_close\"><div class=\"table\"></div></div></div>");
            $("body").append(newDiv);
