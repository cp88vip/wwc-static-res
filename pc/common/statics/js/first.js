...
// var pri_ajaxurl = '/nonAuthority/gameCenter/gameAction'; //ajax 链接
var pri_ajaxurl = 'front/lottery/draw_info.do'; //ajax 链接
var sliderWidth = 150;
// sliderWidth = $("#mySlider").width();
var isownlottery = 0;

function refreshSwatch() {
    console.log('返利算法/ first.js');
    ...
    tmpPrize = tmpPrize + '';
    var $live_game = JSON.parse( sessionStorage.getItem("playGame") );
    var $playLayout = {};
    if ($live_game && $live_game.length > 0) {

      for (var i = 0; i < $live_game.length; i++) {
        var eGame = $live_game[i];
        for (var j = 0; j < eGame.playWayData.length; j++) {
          if (eGame.playWayData[j].playId == playId) {
            $playLayout = eGame.playWayData[j];
            break;
          }
        }
      }
    ...
    if (myPrize[0] == myPrize[1]) {
        ...
        if ($playLayout.name && $playLayout.name.indexOf("组选和值") > -1) {
          // console.log($playLayout);
          //三星直选组合
          // $('#sliderSpanEnd2').html(' | ' + decimal(tmpPrize2 / 4)); //取二星直选复式的赔率
          // $('#B_sliderSpanEnd2').html(' | ' + decimal(tmpPrize2 / 2)); //取二星直选复式的赔率
        }
        ...
        if ($playLayout.name && $playLayout.name.indexOf("三组选和值") > -1) {
          // console.log($playLayout);
          //三星直选组合
          $('#sliderSpanEnd2').html(' | ' + decimal(tmpPrize2 / 4)); //取二星直选复式的赔率
          $('#B_sliderSpanEnd2').html(' | ' + decimal(tmpPrize2 / 2)); //取二星直选复式的赔率
          ...
          $('#sliderSpanEnd2').html(''); //取二星直选复式的赔率
          $('#B_sliderSpanEnd2').html(''); //取二星直选复式的赔率
        }
        // if (playId == 92 || playId == 58) {
        //     //三星直选组合
        //     //计算二星直选复式的赔率
